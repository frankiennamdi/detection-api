...
func (mockEventRepo *MockEventRepository) FindByUsername(username string) ([]*models.Event, error) {
	log.Printf(support.Info, username)
	return mockEventRepo.userEvents, nil
}

func (mockEventRepo *MockEventRepository) Insert(events []*models.Event) ([]sql.Result, error) {
...
func generateEvents(currentEvent *models.Event, increment int) (*models.Event, []*models.Event) {
	closestEvent := newEvent(models.EventInfo{
		UUID:      uuid.New().String(),
		Username:  "john",
		Timestamp: AddTime(currentEvent.ToEventInfo().Timestamp, increment, time.Hour),
		IP:        "1.0.0.0",
	})
	...
	for i := 2; i < 4; i++ {
		newEvent := newEvent(models.EventInfo{
			UUID:      uuid.New().String(),
			Username:  "john",
			Timestamp: AddTime(currentEvent.ToEventInfo().Timestamp, increment*i, time.Hour),
			IP:        "1.0.0.0",
		})
